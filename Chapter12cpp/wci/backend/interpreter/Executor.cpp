    ...
    throw (string)
{
    ...
    call_node->set_attribute((ICodeKey) ID, new NodeValue(program_id));
    NodeValue *node_value =
                icode->get_root()->get_attribute((ICodeKey) LINE);
    int line_number = node_value->value->i;
    call_node->set_attribute((ICodeKey) LINE,
                             new NodeValue(new DataValue(line_number)));
